...
describe('<SignUp />', () => {
  ...
  it('renders the sign up page with a form submission and signs a user up', async () => {
    ...
    await act(async () => {
      ...
      expect(document.title).toEqual('Sign Up - Instaclone');
      await expect(doesUsernameExist).toHaveBeenCalled();
      await expect(doesUsernameExist).toHaveBeenCalledWith('jeremy');

      await waitFor(() => {
        expect(mockHistoryPush).toHaveBeenCalledWith(ROUTES.DASHBOARD);
        expect(getByPlaceholderText('Username').value).toBe('jeremy');
        expect(getByPlaceholderText('Full name').value).toBe('Jeremy Rice');
